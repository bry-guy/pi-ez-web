...
| `merge` button in the chat header | Merge the session's branch into `main`; session continues on `main` |
| `×` on a sidebar session | Close the session **and remove its worktree** |
...
**Worktrees:** the UI now assumes **one worktree per session branch** — closing a session tells the user its worktree is removed and changes on that branch are lost, and the merge button is the sanctioned way to land work first. Evaluate whether one-worktree-per-branch is right at v1 or whether sessions can share the project checkout with a branch-switch guard; if you choose the simpler path, the close and merge copy must change to match, so name the wording change rather than leaving the UI lying.

Define the lifecycle precisely: what `merge` runs (merge vs rebase vs fast-forward-only), what happens on conflict, what `close` does to a dirty worktree, and whether the branch ref survives worktree removal. The UI presents merge as safe and close as destructive — the implementation must actually honor that distinction.
...
- worktree lifecycle: create (branch popover), merge, remove (session close) — including failure paths (merge conflict, dirty worktree on close)
